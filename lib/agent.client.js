                ...
                install: function (installPath, setup, key, jobs, debug, callBack) {
                    ...
                    if (setup.package !== undefined) {
                        ...
                        var req = http.get(options, function (res) {
                            ...
                            res.on('data', function (chunk) {
                                ...
                                try {
                                    ...
                                    api.log.console('There was a problem setting up the library: ' + e.message, 2);
                        ...
                        req.on('error', function (e) {
                            ...
                            api.log.console('There was a problem setting up the library: ' + setup.libraryId + ':' + e.message + '@' + options.host, 2);
                            callBack(-1, new Date().getTime() - startAt, e.message);
                        });
                    } else {
                        if (fs.existsSync(_path))
    ...
    var socket = {
        connect: function (url, callBack) {
            ...
            sock.on('error', function (e) { // something went wrong 
                api.log.console('Lost the connection, retry in ' + options.r + 'ms... ' + (e !== null ? (typeof e === 'object' ? (e[0].code === undefined ? '' : e[0].code) : e) : ''), 2); // + JSON.stringify(e), 2);
                api.jobs.stopAll($j);
                clearInterval(ping);
                callBack();
            });
