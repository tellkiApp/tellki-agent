(function () {
    ...
    var api = {
        ...
        agent: {
            ...
            info: function (properties, callBack) {
    ...
    var socket = {
        connect: function (url, callBack) {
            ...
            sock.on('connection', function () { // connection is established 
                ...
                api.agent.info(options, function (agent) {
                    sock.write(agent);
                });
            });
                ...
                try {
                    ...
                    switch (json.op) {
                        ...
                        case 3: //Execute Command
                            ...
                            run.spawn(options.scriptPath, json, function (resp, code, duration) {
                                sock.write(socket.response(json, resp, code, duration, $a));
                            });
                            break;
                        case 5: //Scheduler Command
                            api.log.console('Scheduler: ' + msg);
                            var pid = run.spawn(options.scriptPath, json, function (resp, code, duration) {
                                sock.write(socket.response(json, resp, code, duration, $a));
                        ...
                        case 100: //Install Library Response
                            api.agent.libraries.install(options.scriptPath, json, function (res, duration, msg) {
    ...
    var run = {
        spawn: function (scriptPath, cmd, callBack) {
            ...
            api.log.console('CMD:' + cmd.cmd + ' ' + cmd.args.toString().replace(/,/g,' '));
            ...
            child.on('error', function (e) {
                ...
                callBack(e.toString(), e.code, options.duration());
                clearTimeout(kill);
            });
    ...
    if (process.argv.length > 2) {
        for (var i = 2; i < process.argv.length; i++) {
            switch (process.argv[i]) {
                case '-id':
                    options.id = process.argv[i + 1];
                    break;
                case '-key':
                    options.k = process.argv[i + 1];
                    break;
                case '-tags':
                    options.tags = process.argv[i + 1];
                    break;
                case '-debug':
        ...
        if (!fs.existsSync(api.conf.file(options.cfgPath, ''))) {
            process.stdout.write('Usage: tellki-agent -key {client_key} [-id {agent_name}] [-tags {tag1,tag2,tagN}]\n');
            process.exit(1);
        }
    // =============================================================================
    ...
    if (options.id !== '') api.log.console('Id: ' + options.id);
