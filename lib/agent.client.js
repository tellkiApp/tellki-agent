(function () {
        ...
        connect: function (url, callBack) {
            ...
            sock.on('data', function (msg) { // received some data 
                ...
                try {
                    ...
                    api.log.console('Something went wrong, unexpected: ' + e, 2);
                    //throw e;
                }
            });
            sock.on('error', function (e) { // something went wrong 
                api.log.console('Something went wrong, must retry in ' + options.r + 'ms...' + JSON.stringify(e), 2);
                api.jobs.stopAll($j);
                callBack();
            });
    ...
    if (invalid) {
        process.stdout.write('Usage and parameters: tellki-agent -key {client_key} [-id {agent_name}] [-tags {tag1,tag2,tagN}] [-uuid {agent_uuid}] [-debug]\n');
        process.exit(1);
    }
    // =============================================================================
