...
try {
    ...
    console.error($nv + ' is not found');
    var npm = require("npm");
    npm.load({
        loaded: false
    }, function (err) {
        npm.commands.install([$nv], function (er, data) {
            install();
        });
            ...
            switch (process.argv[i]) {
                ...
                case '-status':
                    if (process.platform === 'win32') {
                        lib.list(function (list) {
                            for (var svc in list) {
                                if (list[svc].SessionName === 'Services' && list[svc].ImageName === 'tellkiagent.exe') {
                                    console.log(list.name + (list[svc].PID !== undefined ? 'Running with PID:' + list[svc].PID : 'Stopped'));
                                    return;
                                }
                            }
                        }, true);
