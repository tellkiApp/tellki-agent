...
var nw = 'tellki-node-windows', nwv = nw + '@0.0.1',
    path = require('path'),
    fs = require('fs');
...
try {
    ...
    console.error(nwv + ' is not found, installing...');
    var npm = require("npm");
    npm.load({
        loaded: false
    }, function (err) {
        npm.commands.install(path.join(path.dirname(fs.realpathSync(__filename)), '../'), [nwv], function (er, data) {
            if (er) {
                console.log(er);
                process.exit(-1);
            } else
