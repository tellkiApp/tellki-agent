...
var options = {
	...
	redirectToFile : '>> {{LOG_FILE_PATH}} 2>&1',
...
function processInitFile(args) {
	...
	// Setup replace vars
	var pathS = path.join(fs.realpathSync(__filename), '../..');
	var serviceNameS = options.serviceName;
	var pidFileS = options.agentPidFile;
	var startCmdS = options.agentStartCmd;
	var redirectToFileS = options.redirectToFile.replace(/{{LOG_FILE_PATH}}/g, pathS + '/log/service.log');
	...
	data = data.replace(/{{REDIRECT_TO_FILE}}/g, redirectToFileS);
...
case \"$1\" in\n\
	start)\n\
		if [ ! -s $PIDFILE ]; then\n\
			...
			RETURNCODE=1\n\
		elif [ $(ps -o args -p $(cat $PIDFILE) | grep tellki | wc -l) -gt 0 ]; then\n\
			echo \"$SERVICE_NAME is already running\"\n\
			RETURNCODE=1\n\
		else\n\
			echo \"Starting $SERVICE_NAME\"\n\
			su - $AGENTUSER -c \"{{STARTCMD_S}} {{REDIRECT_TO_FILE}} &\"\n\
			\n\
			if [ -f /etc/SuSE-release ]; then\n\
				rc_status -v\n\
	...
	stop)\n\
		if [ ! -s $PIDFILE ]; then\n\
			echo \"$PIDFILE not found or empty\"\n\
			RETURNCODE=1\n\
		elif [ $(ps -o args -p $(cat $PIDFILE) | grep tellki | wc -l) -eq 0 ]; then\n\
