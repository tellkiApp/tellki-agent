...
function checkInstaledService() {
	...
	fs.exists(options.serviceFilePath, function (exists) {
		if (exists) {
			console.log('Init file found, please run with -uninstall first');
			process.exit(1);
		} else {
			activateAgent(args);
		}
...
function setService(command, callback) {
	...
	run(options.cmdService,
		function onExit(code) {
			if (code === 127 || code === -1) {
				console.log('Init not found: ' + options.serviceFilePath);
				process.exit(1);
			} else if (code === 0 || code === 1) {
				callback();
			} else {
				console.log('Error sending ' + command + ' to tellki-agent service');
				process.exit(1);
			}
		});
}
...
case \"$1\" in\n\
  start)\n\
		if [ $(ps up $(cat $PIDFILE) | grep tellkiagent | wc -l) -gt 0 ]; then\n\
	...
	\n\
  stop)\n\
		if [ $(ps up $(cat $PIDFILE) | grep tellkiagent | wc -l) -eq 0 ]; then\n\
